...
export function Birds2(props) {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF("/models/birds2.glb");
  const { actions } = useAnimations(animations, group);

  const [positionX, setPositionX] = useState(props.firstPosition);
...
// useGLTF.preload("/models/birds2.glb");
